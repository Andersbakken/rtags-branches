...
Path::VisitResult GRTags::visit(const Path &path, void *userData)
{
    ...
    case Filter::Source: {
        ...
        if (!parsed) {
            ...
            debug() << path << "seems to be up to date. Parsed at" << ByteArray::timeToString(parsed, ByteArray::DateTime)
                    << "last modified at" << ByteArray::timeToString(path.lastModified(), ByteArray::DateTime);
        }
        break; }
    }
    return Path::Continue;
...
void GRTags::dump()
{
    ...
    for (Map<uint32_t, time_t>::const_iterator it = mFiles.begin(); it != mFiles.end(); ++it) {
        ...
        if (it->second) {
            error() << "  " << path << ByteArray::timeToString(it->second, ByteArray::DateTime);
        } else {
            error() << "  " << path;
        }
    }
